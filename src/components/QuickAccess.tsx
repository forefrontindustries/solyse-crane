        ...
        <div className="flex w-full lg:w-auto">
          {quickLinks.map((link, i) => (
            <a
              key={link.label}
              href={link.href}
              className="flex items-center gap-2.5 py-4 px-6 lg:px-8 text-center transition-all duration-200 hover:bg-[#ebeef3] group"
              style={{ borderRight: i < quickLinks.length - 1 ? "1px solid #e2e6ed" : "none" }}
            >
              <span className="text-slate-500 group-hover:text-secondary transition-colors">
                {link.icon}
              </span>
              <span className="text-[13px] font-medium text-slate-600 group-hover:text-foreground transition-colors">{link.label}</span>
            </a>
          ))}
        </div>
      </div>
